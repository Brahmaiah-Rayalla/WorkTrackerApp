...
import app.models.TaskAggregation;
import app.models.User;
import app.models.Userwork;
import app.repositories.UserRepository;
import app.repositories.WorkRepository;
...
public class UserService implements UserDetailsService {
	...
	public User register(User user) {
		...
		if (this.repo.findOneByUserName(user.getUserName()) == null
				&& this.repo.findOneByEmail(user.getEmail()) == null) {
	...
	public void updateUser(String userName, User newData) {
		this.repo.updateUser(userName, newData.getEmail(), newData.getFirstName(), newData.getLastName(),
				newData.getAddress(), newData.getCompanyName(), newData.getGroupName());
	}
	...
	public void saveWork(Userwork work) {
		this.workRepo.save(work);
	}

	public Long getTaskCount() {
		return workRepo.getTaskCount(getLoggedInUser().getUserName());
	}

	public Long[] getHoursAndSubmissionsCount() {
		Long[] result = new Long[2];
		List<Object[]> hoursAndSubmissionsArr = workRepo.getHoursAndSubmissionCount(getLoggedInUser().getUserName());
		for (Object[] item : hoursAndSubmissionsArr) {
			result[0] = Long.parseLong(item[0].toString());
			result[1] = Long.parseLong(item[1].toString());
		}
		return result;
	}

	public List<TaskAggregation> getTaskInfo() {
		List<Object[]> aggreArray = workRepo.getTaskInfo(getLoggedInUser().getUserName());
		List<TaskAggregation> listAggre = new ArrayList<>();
		for (Object[] item : aggreArray) {
			listAggre.add(new TaskAggregation(null, (String) item[0], Long.parseLong(item[1].toString())));
		}
		return listAggre;
	}

	public List<Userwork> getWorkByUserName() {
		return workRepo.findTop10ByUNameOrderByCurrDateDesc(getLoggedInUser().getUserName());
	}
