...
public class UserController {
	...
	public String home(ModelMap map) {
		Long[] hoursnAndSubmission = userService.getHoursAndSubmissionsCount();
		map.put("taskAggregations", userService.getTaskInfo());
		map.put("tasksCount", userService.getTaskCount());
		map.put("hoursCount", hoursnAndSubmission[0]);
		map.put("submissionsCount", hoursnAndSubmission[1]);
		map.put("workList", userService.getWorkByUserName());
		return "dashboard";
	}
	...
	public String work(Userwork userWork) {
		return "user/work/submit-work";
	}

	@RequestMapping(value = "/user/work", method = RequestMethod.POST)
	public String postWork(@Valid Userwork userWork, BindingResult result) {
		if (result.hasFieldErrors("phase")) {
			return "redirect:/user/work?phase";
		}
		/*
		 * if(result.hasFieldErrors("status")){ return "redirect:/user/work?status"; }
		 */
		if (userWork.getHoursWorked() == 0) {
			return "redirect:/user/work?hoursWorked";
		}
		userWork.setUserName(userService.getLoggedInUser().getUserName());
		userWork.setCurrentDate(LocalDateTime.now());
		userService.saveWork(userWork);

		return "redirect:/user/work?submitted";
	}
	...
	// @RequestMapping(value = "/user/send-user-details", method =
	// RequestMethod.POST)
